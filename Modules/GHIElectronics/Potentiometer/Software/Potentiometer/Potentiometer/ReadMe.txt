...
1) GadgeteerHardware.xml - defines some parameters about your module.
...
1)  For each of the two NETMF projects (with the suffixes _41 and _42), in the Solution Explorer, right click and choose Properties, then change 
    ...
    Also change "GTModuleName" to a name for your module with no spaces or punctuation, in AssemblyName. 
    These names should match what you print on the module PCB.  

2)  Do THREE Quick Replaces (Ctrl-H) on the entire solution to customise the template to your module name and manufacturer name.
    ...
    Replace "GTModuleName" with the name of your module without any spaces or punctuation.
    Replace "ManufacturerFullName" with the name of your institution, which may include spaces or punctuation.

3)  Edit the [ModuleName].cs files to implement software for your module.
...
6)  Optionally, change Image.jpg to a good quality top-down image of the module with the socket side facing up,
...
8)  Build in Release configuration to build the module installer in the installer project's bin\Release\Installer directory!
    Install this and check that it appears correctly in the Control Panel's Add/Remove Programs window.

9)  Now open a new Gadgeteer App (for the right version(s) of NETMF) and your module should appear in the designer.
