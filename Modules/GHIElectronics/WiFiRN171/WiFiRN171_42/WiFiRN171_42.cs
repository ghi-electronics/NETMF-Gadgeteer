...
namespace Gadgeteer.Modules.GHIElectronics
{
    ...
    public class WiFiRN171 : GTM.Module
    {
		...
		public delegate void HttpRequestReceivedHandler(HttpSession request);
        /// <summary>
        ...
        protected virtual void OnHttpRequestReceived(HttpRequest request)
        {
            ...
            {
				var stream = new HttpSession(request, _wifly);
        ...
        private void _ParseBufferForHttp(string buffer)
        {
            ...
            if (HttpStream)
            {
                ...
                else if ((index = HttpBuffer.IndexOf("\r\n\r\n")) >= 0)
                {
                    ...
                    if (current_request.Type == HttpRequest.HttpRequestType.GET)
                    {
                        HttpBuffer = "";
                        HttpStream = false;
					...
					else if (current_request.Type == HttpRequest.HttpRequestType.POST)
                    {
        ...
        private HttpRequest _ParseHttpHeader(string buffer)
        {
            ...
            if (buffer.IndexOf("GET") >= 0)
				request.Type = HttpRequest.HttpRequestType.GET;
            else if (buffer.IndexOf("POST") >= 0)
				request.Type = HttpRequest.HttpRequestType.POST;
            ...
            while ((index = buffer.IndexOf("\r\n")) >= 0)
            {
                ...
                if (request.Type == HttpRequest.HttpRequestType.POST && keep_check)
                {
                    ...
                    {
						int len = 0;
						StringToInt(line_buffer.Substring(16), out len);
						request.PostLength = len;
		...
		public class HttpResponse
		{
			...
			public void Send(byte[] document, int offset, int count, bool sendHeader)
			{
				...
				if (sendHeader)
					this.stream.Write(Encoding.UTF8.GetBytes(this.HeaderToString()));
